...
description: "AI prototyping speeds up design dramatically, but when early prototypes look real, feedback can shift from defining the problem to critiquing the interface."
pubDate: "2026-03-13"
author: "Anna Kahrs"
...
One of the first things I noticed was how quickly the prototype changed the nature of feedback.
...
Another effect of AI prototyping is that it can bypass an important part of design system thinking. In traditional UX workflows, components emerge gradually. A card pattern appears. Then a list layout. Then a page structure. Each step gives teams a chance to talk about reuse, hierarchy, and consistency. The system becomes visible as it forms.

With AI-assisted prototyping, however, the system can appear all at once. The interface materializes as a complete environment before those foundational conversations happen. Stakeholders experience the assembled product instead of the building blocks that created it and the result is a subtle inversion of the process.

Instead of defining components and patterns first, teams start reacting to the finished-looking interface which can make it harder to talk about the system beneath it.

### Feedback Becomes Harder to Structure

There was also a practical challenge. When we gather feedback on design work, tools like Figma provide a shared environment for commenting, annotation, and discussion. Feedback attaches directly to frames and components. But a coded prototype does not always offer that structure. In this case, I had to create a separate feedback form just to collect reactions. The prototype was easy to explore, but harder to discuss collaboratively.
...
Sometimes the system needs to be discussed before the interface appears. AI prototyping is incredibly powerful, and I will absolutely keep using it. It accelerates exploration and makes ideas tangible much faster than traditional workflows. But it also introduces a new responsibility, where designers now have the ability to generate artifacts that look like finished products long before the thinking behind them is finished.

And that means the challenge is no longer just designing interfaces, it is designing the conditions for the right conversations to happen before those interfaces feel real.